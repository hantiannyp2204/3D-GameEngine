...
{
    public class MissleManager : UnityEngine.MonoBehaviour
    {
        [Tooltip("Prefab")]
        [SerializeField] private MissleVFX rocketPrefab;
        ...
        private void Awake()
        {
            objectPool = new ObjectPool<MissleVFX>(CreateProjectile,
                OnGetFromPool, OnReleaseToPool, OnDestroyPooledObject,
                collectionCheck, defaultCapacity, maxSize);
        }

        // invoked when creating an item to populate the object pool
        private MissleVFX CreateProjectile()
        {
            MissleVFX projectileInstance = Instantiate(rocketPrefab);
        ...
        private void OnReleaseToPool(MissleVFX pooledObject)
        {
            pooledObject.gameObject.SetActive(false);
        }

        // invoked when retrieving the next item from the object pool
        private void OnGetFromPool(MissleVFX pooledObject)
        {
        ...
        private void OnDestroyPooledObject(MissleVFX pooledObject)
        {
        ...
        public void Render(BaseWeapon currentWeapon,Transform muzzlePos, Vector3 direction)
        {
            ...
            MissleVFX fireVFXObject = objectPool.Get();
